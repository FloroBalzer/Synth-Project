...
void loop() {
  ...
  if (millis() > next) {
    ...
    for (uint8_t i = 0; i < 3; i++){
      ...
      String binaryVal = String(val, BIN);
      uint8_t notkey = ~val;
      String binnotkey = String(notkey, BIN);
      //Serial.println(binnotkey);
      int notkeyint = strtol(binnotkey.c_str(), NULL, 2);
      //Serial.println(notkeyint);
